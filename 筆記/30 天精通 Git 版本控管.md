...
* 做一個小功能修變就建立版本，這樣才容易追蹤變更
* 不要累積一大堆修改後才建立一個「大版本」
* 有邏輯、有順序的修正功能，確保相關的版本修正可以按順序提交，這樣才更於追蹤

> 大多不外乎 commit 刪除、改變順序、修改
...
## 修正 commit 過的版本歷史紀錄 part 2

> 要怎麼說呢？還原就機制上來說，就是合併，一樣會有衝突。主要是因為透過新增版本來改更內容。



## 修正 commit 過的版本歷史紀錄 part 3

> 簡單來說，就是僅想要分支的一部分，而不要全部。

> 沒有實作，主要就是從分支中取出一個 commit ，但不確定是不是全部內容，還是一部分。



## 修正 commit 過的版本歷史紀錄 part 4

嚴格上來說 `git revert` 與 ` git cherry-pick` 並不算「修正版本歷史紀錄」比較像套用。

> `git rebase NAME` 感覺還好，在例子中比較像是為了讓分支於主幹更合理所使用的



## 修正 commit 過的版本歷史紀錄 part 5

上一章的 Rebase 也可以用來修改特定分支線上的版本資訊。

> 上一章裡面有講到千萬不要透過 Rebase 修改遠端儲存的分支。



> 這一章再次強調千萬不要用 Rebase 的請況，所以內容不是很仔細看，主要還是像上一章講的一樣 Rebase 的功能可以修改、移動…之類的介紹。



## 使用 GitHub 遠端儲存庫 - 入門篇

沒有版本最重要的就是不要勾選 Initialize this repository with a README 和保留預設的 None。



> 用最簡單的方法就可以使用儲存庫，所以這一章對我來說只是介紹不同的類型如何上傳而已
...
> 這一章很重要，對於大多數使用 git 的人，如何開分支和命名，有很重要的啟發。而且只前暫存的觀念，原來在我使用時已經用過了，算是了解它背後的技術
...
> 沒有用到，可以簡單來說就是讓 Subversion 可以建立一份 Git 儲存資料，而且兩者可以通用，但作者建議轉過去就不要傳回來了，要不然有點多此一舉。



## 分享工作中幾個好用的 Git 操作技巧

> ssh 可以不用密碼（但有點麻煩），其他幾個有點對我目前來說沒什麼用。GitHub Graph 圖表的介紹倒是不錯。



> 說實在有很多地方都沒有實作，但整體來說感興趣的都有實作一下，到目前這個分支算已經看完了。 good!!



## 目前使用心得

> `.gitignore` 在 sourceTree 使用方法，有很好的心得。
>
> `git reflog` 目前是修改 commit 最好的指令。